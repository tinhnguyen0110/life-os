...
- **When you verify on the container, leave it UP (detached) + rebuild for code.** <!-- Added sprint 8: backend ran `docker compose up backend` (foreground) for its verify 3×, which EXITS when the tool-shell ends → the canonical stack goes DOWN, breaking FE/tester's live-verify (they hit HTTP-000). --> Use **`docker compose up -d --build backend`** (detached + rebuilt-for-code-changes), NOT foreground `docker compose up`. Foreground exits with your shell → stack down. `--build` because the backend image does NOT hot-reload Python (a recreate without `--build` serves the OLD code — the stale-image trap). After your container verify, the stack must stay up for the next role. See memory `host-file-source-must-mount` (rules 5-7), `dev-server-ports`.
- **When you change a function's BEHAVIOR, reconcile its DOCSTRING in the same edit + pin the decided behavior with a teeth-test.** <!-- Added sprint 8: restore's docstring said "clear lesson" but the code (after the preserve ruling) kept it — I edited abandon_keys, not the docstring above it → team-lead read the lying doc + chased a false bug. Same class as S6 pinned (edited the change-list, not the full surface). --> The docstring is part of the change surface, not separate prose — a stale doc is a CLAIM that lies to the next reader. For any decide-and-log behavior (preserve-vs-clear, snapshot-vs-live, include-vs-skip), write a teeth-test that ENCODES the ruling (RED if the behavior flips) — that pins it even if the prose drifts. Doc after final code + a behavior-test on the decided rule. See memory `behavior-test-not-field-read`, `test-where-the-reader-greps`.
- **Derived stat over OPTIONAL inputs → decide include / skip / None EXPLICITLY (don't let missing default to 0).** <!-- Added sprint 8: avgPeak treated missing abandonedProgress as 0 → skewed the mean low; ruling = skip. A math-correctness call I should've caught from reasoning (not real-data). --> When you aggregate (avg/sum/count/ratio) over a field that can be None/missing, state the policy per aggregate: skip-missing (avg of present only), treat-as-0, or return None. None-as-0 silently skews means/ratios. Carry the inputs ({sum,count}) so it's self-describing. This + verify-on-real-data-before-freeze are the two places a first build is weakest (S7 MiniMax, S8 avgPeak — real-data + derived-stat edges).

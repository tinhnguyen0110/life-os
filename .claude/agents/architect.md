...
```markdown
[Sprint X — Task #N]
## Read first (memory — MANDATORY, EVERY dispatch, HARD GATE): the FIRST line of EVERY dispatch (backend/FE/tester, every task, every sprint) names 2-4 role-relevant memory files the teammate MUST `Read` before starting. Per-DISPATCH, NOT per-sprint-once — a dispatch without it is INCOMPLETE. team-lead gate (op-model §2.2): team-lead greps each dispatch for the line + role-correctness at scope-confirm and will NOT greenlight without it. The index loads but FILES are read on-demand — a spawned teammate does NOT auto-read them. Role-target it: backend → `schema-freeze-gate`, `unhandled-errors-not-green`, any decided-algorithm/`abandon-orthogonal-to-health` memory for this feature, + the existing-code paths to build ON · FE screen → `mock-diff-catches-dropped-feature`, `unhandled-errors-not-green`, `dev-server-ports` · tester → `workaround-then-ask-why-accepted`, `verify-live-app-not-just-suite`, `behavior-test-not-field-read`, `verify-with-the-distinguishing-case` (multi-axis stats). Name the FILES explicitly ("Read memory `schema-freeze-gate` + `unhandled-errors-not-green` first"). The FIRST net for solved-mistake recurrence — proven repeatedly (S6 `pinned` dropped; S10A backend reached for `health=="dead"` on idle-hunter, closed by the abandon-orthogonal memory in its `## Read first`). Without the named file, the bug ships.
## Context (1 paragraph)
## Scope (IN / OUT lists)

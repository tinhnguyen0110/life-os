...
└── life-os/                      # ⭐ APP THẬT — repo git riêng
    └── template/                 # MOCK design reference (GIỮ NGUYÊN, bất biến)
        └── Life Command/         #   data.js (schema gốc) + screens-*.js + app.css (design tokens)
```

App thật `life-os/` là repo độc lập — chính nó sau cũng là 1 dự án được track trong app (ăn dogfood). Mock `template/Life Command/` sống ngay trong repo làm design baseline bất biến.
...
- Design baseline: mock `template/Life Command/` (đã duyệt màu+layout)

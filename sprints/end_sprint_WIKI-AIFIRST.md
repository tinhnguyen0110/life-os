...
- **Vault "new note" CTA repointed to /wiki/proposals (Nhật ký AI)** — *why*: the old top-bar inbox CTA is gone; the audit-log is the most relevant adjacent destination for that button — *how to change*: edit the link in page.tsx.
- **CORRECTION (user-flagged, was wrong above): note-CREATE + folder-CREATE ALREADY EXIST in-app** via `WikiExplorer.tsx` (#127-W3): toolbar "＋ Thư mục" (root folder) + "Nhập" (import .md/.txt), and the per-folder ⋯ menu has "new-note" (createWikiNote into the folder → opens the new note at /wiki/{id}), "Thư mục con mới" (nested folder), rename/move/delete/import-here. So the wiki is NOT missing a create flow — my original kickoff only checked the inbox create path and missed the explorer. No feature gap; nothing to build for create.
